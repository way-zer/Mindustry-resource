	...
	<PageHeader title="游戏下载与安装">
		<template #actions>
			<el-switch
				v-model="useMirror"
				active-text="使用镜像加速"
				inactive-text="不使用镜像加速"
			/>
		</template>
		<ReleaseList.define v-slot="{ list }">
			<el-collapse v-if="list.length > 0" accordion>
				<el-collapse-item
					v-for="item in list"
					:key="item.tag_name"
					:name="item.tag_name"
				>
					<template #title>
						<a :href="item.html_url" target="_blank" rel="nofollow">{{
							item.tag_name
						}}</a>
					</template>
		...
		<el-card header="官方 正式版">
			<div class="alert">
				<div class="col-span-full">
					数据源为官方<a class="link" href="https://github.com/Anuken/Mindustry"
						>Github</a
					>，只提供电脑版(需Java)和服务端
				</div>
				<details class="col-span-full">
					<summary>
						apk等版本请前往
						<a
							href="https://anuke.itch.io/mindustry"
							target="_blank"
							rel="nofollow"
						>
							官方itch下载站
		...
		<el-card header="国产第三方客户端 MindustryX">
			<div class="alert">
				<div class="col-span-full">
					其中APK为手机版，desktop为PC版(需java)，Loader为MOD(需原版)，server为服务端
				</div>
				<div class="col-span-full">
					开源地址:
					<a class="link" href="https://github.com/TinyLake/MindustryX"
						>Github</a
					>
				</div>
			</div>
