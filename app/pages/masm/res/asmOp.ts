...
function radar(
	...
	if (condition.length > 3) throw "condition can't be more than 3."
	while (condition.length < 3) condition.push("any")
	const result = variable()
	builder.line(
		`uRadar ${condition.join(" ")} ${sort} ${from} ${desc ? 1 : 0} ${result}`,
	)
	return result
}
...
function uRadar(
	...
	const result = variable()
	builder.line(
		`uRadar ${condition.join(" ")} ${sort} 0 ${desc ? 1 : 0} ${result}`,
	)
	return result
}
