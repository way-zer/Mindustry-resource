  ...
  <div class="p-6 max-w-screen-lg mx-auto">
    ...
    <el-card class="mb-6">
      ...
      <el-descriptions v-if="playerStats" :column="6" border>
        <el-descriptions-item label="QQ号" span="3">{{ playerStats.qq }}</el-descriptions-item>
        <el-descriptions-item label="最终得分" span="3">{{ playerStats.finalScore }}</el-descriptions-item>
        <el-descriptions-item label="比赛次数" span="2">{{ playerStats.matchCount }}</el-descriptions-item>
        <el-descriptions-item label="胜场数" span="2">{{ playerStats.winCount }}</el-descriptions-item>
        <el-descriptions-item label="胜率" span="2">{{ playerStats.winRate }}%</el-descriptions-item>
      </el-descriptions>
      <div v-else class="text-center text-gray-500">请输入有效的 QQ 号以查看统计信息</div>
    </el-card>
    ...
    <el-card class="mb-6">
      <template #header>
        ...
        <div class="flex justify-between items-center">
          <div class="text-lg font-bold">比赛记录 (共 {{ filteredMatches.length }} 场)</div>
          <el-pagination
      ...
      >
        ...
        <div class="flex justify-between items-center mb-2">
          <div class="text-base font-bold">比赛 {{ currentStart + index + 1 }}</div>
          <div class="flex gap-2">
            <el-tag type="info">地图 {{ match.map }}</el-tag>
            <el-tag type="warning">时长 {{ (match.duration / 60).toFixed(2) }} 分钟</el-tag>
        ...
        <div class="flex justify-between items-center gap-4">
          ...
          <div
              ...
              :class="match.users[0].rank === 1 ? 'border-green-500 bg-green-50' : 'border-gray-200'"
          >
            <el-tag
          ...
          <div
              class="flex-1 rounded p-3 relative border"
              :class="match.users[1].rank === 1 ? 'border-green-500 bg-green-50' : 'border-gray-200'"
          >
            <el-tag
